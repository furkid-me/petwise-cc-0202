        ...
        {plans.map((plan) => (
          <Card
            ...
            className={`cursor-pointer transition-all ${
              ...
              plan.id === 'PREMIUM' ? 'bg-gray-950 text-white border-gray-950' : ''
            }`}
          ...
          >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`rounded-full p-2 ${
                    plan.id === 'PREMIUM' ? 'bg-white/10 text-white' : plan.highlighted ? 'bg-primary/10 text-primary' : 'bg-muted'
                  }`}>
                    {plan.icon}
                  </div>
                  <div>
                    ...
                    <CardDescription className={plan.id === 'PREMIUM' ? 'text-white/60' : ''}>{plan.description}</CardDescription>
                  </div>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pb-2">
              <div className="mb-4">
                ...
                <span className={plan.id === 'PREMIUM' ? 'text-white/60' : 'text-muted-foreground'}> / {plan.period}</span>
              </div>
                ...
                {plan.features.map((feature, index) => (
                  <li
                    ...
                    className={`flex items-center gap-2 text-sm ${
                      feature.included
                        ? ''
                        : plan.id === 'PREMIUM' ? 'text-white/40' : 'text-muted-foreground'
                    }`}
                  >
            ...
            {selectedPlan === plan.id && plan.id !== currentPlan && plan.id !== 'FREE' && (
              <CardFooter>
                <Button
                  className="w-full rounded-full bg-gray-950 text-white hover:bg-gray-800"
