  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm p-4 flex items-center justify-between">
        ...
        <h1 className="text-lg font-semibold">檢驗報告</h1>
        <button
          onClick={() => router.push('/examination-records/new')}
      ...
      <div className="p-4 space-y-3">
        ...
        {records.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">還沒有檢驗記錄</p>
            <button
              onClick={() => router.push('/examination-records/new')}
          ...
          records.map((record) => (
            <div key={record.id} className="bg-white rounded-xl shadow-sm p-4">
              <div className="flex items-start justify-between">
                <div>
                  <span className="inline-block bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full mb-1">
                  ...
                  {record.reportUrl && (
                    <a href={record.reportUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 underline mt-1 block">查看報告</a>
                  )}
                </div>
                <span className="text-sm text-gray-400">{record.examinationDate?.slice(0, 10)}</span>
              </div>
            </div>
          ))
        )}
